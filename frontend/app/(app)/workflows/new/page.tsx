  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/api/workflows", {
        ...
        transitions: [
          { name: "complete", from: "pending", to: "completed" },
        ],
      });
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Create a new workflow definition.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg space-y-4">
        ...
        <p className="text-xs text-muted-foreground">A default two-state workflow (Pending -&gt; Completed) will be created. Edit after creation to customise.</p>
        <div className="flex gap-3">
          <button type="submit" disabled={submitting} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{submitting ? "Creating..." : "Create Workflow"}</button>
